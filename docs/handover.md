...
最終更新: 2026-08-04(コミット 36c8794 時点)

**いま何をしていたか(セッション移行直後に読む)**
- **直前は実装ではなく「壁打ち(相談)」をしていた**。テーマはトレーナーの
  パーソナライズ(見た目の画像生成 + 性格の成長型メモリー)。方針は固まったが
  **実装は着手していない**。決まった内容は「9. 未完・今後の予定」の冒頭にまとめてある
- **⚠️ この壁打ちの途中で、指示が出ていないのに記憶一覧UIを実装・プッシュしてしまい、
  岩田さんから「かってに作業進めないでください、壁打ちしてただけなので」と指摘を受けて
  revertした**(f1a6bd5 を 36c8794 で打ち消し)。
  **相談中は相談に徹すること。実装は「作ってください」と明示的に言われてから**。
  実装内容自体を否定されたわけではないので、指示が出れば `git show f1a6bd5` で復元できる
- 2026-08-04に完了した作業: バグ修正3件(筋トレセット500件超で消える / チャットが時系列に
  並ばない / 体重を訂正してもグラフに反映されない)、UI調整3件(入力バー縮小・PFCカード
  余白削減・キーボード表示時の余白解消)、ログアウトボタン、サイドバーの「トレーナー」ボタン(中身は空)
- その前のテーマ: 記録ページ(`/log`)の食事タブ(2026-08-03完成)。次は同じ構成で筋トレ・体重タブ
- **同期状態(2026-08-04 時点)**: `lovable/main` = **2347968**(体重訂正の修正まで)。
  origin だけにあるのは **dd8c67d(サイドバーのトレーナーボタン)** と、
  打ち消し合う f1a6bd5 / 36c8794 の2つ。つまり**実質「トレーナーボタン」1件だけが未反映**。
  lovable/main は origin の祖先なので、同期指示が出れば fast-forward で送れる
  (**「同期して」と言われるまで絶対に送らない**)
- **Artifactは現在「オンボーディング確認用」の版を表示中**(通常のアプリ版ではない)。
  詳細は「3. プレビュー(Artifact)」の後半を参照。**8/4の作業分はまだ反映していない**
...
## 4. 実装済み機能(2026-08-04 36c8794 時点)
...
- ここに入る予定のものがもう1つある: **「トレーナーが知っていること」= 記憶一覧**
  (9章のパーソナライズ(B)参照)。一度実装したが指示前だったためrevert済み
...
- **体重は1日1件に絞ってから表示する**(`lib/weight.ts` の `latestPerDay`。
  ...
  - **根治用のSQLは岩田さんへ受け渡し済み(2026-08-04)**: DB関数 `confirm_pending_action` の
    `weight.create` を「常にinsert」から「同じJST日の手入力行があればupdate、無ければinsert」へ
    変更するパッチ。ローカルのPostgreSQL 16に本番同等のスキーマを立てて10ケース検証済み
    (バグ再現・別日は分離・JST日境界・3回訂正しても1行・healthkit由来の行は保護・
    他4種のaction_typeとexpired経路が無傷)。**岩田さんがSupabaseダッシュボードから自分で適用する**方針
  - ⚠️ 適用済みかどうかは未確認。体重まわりを触るときは現在の関数定義を先に確認すること
  - 岩田さんの好み: **SQLエディタはなるべく使わず、どのテーブルを直したか分かる形にしたい**。
    受け渡しは**ファイル添付ではなくチャットにコピペできる本文**で出すこと。
    差し替え前の元の定義も必ず本文で残す(ロールバック用)
...
- **「Todo教えて」と言われたら「8. 本日のToDo」の表を出す**(岩田さんからの明示的な依頼。
  進捗も併せて更新して見せる)
- **相談(壁打ち)と実装依頼を混同しない**。「壁打ち相手になって」「〜したいんですよね」
  「どう思いますか」は相談であって着手の合図ではない。**方針が固まっても、
  「作ってください」「実装しましょう」と言われるまで実装しない**
  (2026-08-04に勝手に実装してrevertになった実例あり。冒頭の注意書き参照)
- **実害ベースで説明する**。「設計が良くない」ではなく「何件たまると誰に何が起きるか」。
  抽象的なコード品質の話は判断材料にならないと明言されている
- デザインは勝手に変えない。作り直しの指示は普通にある(カメラ機能は3回作り直した)ので気にせず対応
- アニメーションの速度・タイミングは体感で細かく調整指示が来る(数値をすぐ変えられる形で実装しておく)

## 8. 本日のToDo(岩田さん作成・「Todo教えて」と言われたらこれを出す)

2026-08-04に岩田さんから共有されたリスト。**原文のまま**残す。

```
トレーナー設計
* 初期設定でタイプ・性別・性格を選択(不要なら非表示も可)
* ホーム画面に動きのあるアバターを常駐させ、存在感を演出
* 朝の挨拶通知、記録もトレーナーが管理している演出(LINEスタンプ的な人格表現)

AI機能強化
* 初心者(想定9割)向けに専門用語には参考動画リンクを自動提示
* 機器の写真から使い方を回答
* 運動能力を会話から把握し、個別最適化メニューを提案

本日のToDo
1. トレーナー選択画面の追加
2. ログアウトボタン実装+ベータ版リリース
3. アバターUI組み込み
4. YouTube動画自動提示機能
5. 朝の通知機能
6. 画像生成AI連携の複製・応用
...
**担当レイヤーの分類と進捗**(◎=主担当 ○=作業あり △=軽微)

| # | 項目 | Dify | Supabase | フロント | 状態 |
|---|---|:--:|:--:|:--:|---|
| 1 | トレーナー選択画面の追加 | △ | ○ | ◎ | 未着手(土台は半分あり) |
| 2 | ログアウト+ベータ版リリース | — | △ | ◎ | **ログアウト完了**・リリース準備が残り |
| 3 | アバターUI組み込み | — | — | ◎ | 未着手(静止画のみ有) |
| 4 | YouTube動画自動提示 | ◎ | ○ | △ | 未着手(**表示の器は完成済**) |
| 5 | 朝の通知機能 | — | ◎ | ○ | 未着手(3層すべてに跨る) |
| 6 | 画像生成AI連携の複製・応用 | ? | ? | ? | **内容が未確定**(要確認) |
| 7 | マネタイズ3プランの確定 | — | — | — | 決定事項(実装ではない) |

- **#1**: `trainer_settings` に `trainer_name` / `trainer_style` / `explanation_level` が既にあり、
  トレーナー4人と選択UIも実装済み。**性別の列追加**と**保存先を端末localStorageからDBへ切り替え**れば形になる
- **#4**: `onboarding_contents` テーブル + `LearningContentCard` + `VideoPlayerDialog` が既に動く。
  **Difyが「用語→動画」を返せるようになれば一気に動く**状態。柴崎さんへ先に投げておくと待ち時間を使える
- **#5**: iOSのWeb Pushは**ホーム画面に追加したPWAでのみ動作**。Service Workerは現在ゼロ。単独で1日確保推奨
- **#6**: 「複製・応用」が何を指すか未確認のまま。トレーナーのアバター生成の話か、
  別プロジェクトの移植かを岩田さんに聞く必要がある
- **#7**: 画像生成のコスト試算(下記)と繋がる。無料=4プリセット / 有料=生成解放 という切り分け案を提示済み

## 9. 未完・今後の予定

**トレーナーのパーソナライズ(2026-08-04の壁打ちで方針だけ決定・実装は未着手)**

「あなたに専属トレーナーを」というコンセプトの中核。**見た目**と**性格**の2本立て。
以下は岩田さんと合意済みの方針。**着手の指示はまだ出ていない**。

*(A) 見た目 — 画像生成AIでユーザーごとのトレーナーを作る*
- **オンボーディング時に1回だけ6枚まとめて生成**(素立ち・ガッツポーズ・よしよし 等のポーズ差分)。
  あとからまとめて作り直すことはしない。**1枚だけ気に入らない場合の個別作り直しは都度課金**
- トレーナーページを開いたときだけ全身が見える演出。動きは控えめ(表情・ポーズ差分の切り替え程度)
- モデルは **character consistency(同一人物を保った編集)** 対応が必須。
  Flux Kontext Pro が用途に最も合う(1枚 約¥4.5〜6)。他に Kontext Max(¥7.5〜12)、
  Nano Banana/Gemini(¥6〜20)。アプリ表示用なので**4K等の高解像度は不要**
- **概算コスト: ユーザー1人あたり ¥40〜150**(6枚+やり直しバッファ2〜3割)。
  オンボーディング時の1回きりなので、サブスク原価としては誤差の範囲。
  個別作り直しは原価¥4〜20 → ¥100〜300で提供すれば5〜15倍のマージン
- ⚠️ **これは相場からの概算**。実際の見積もりはサービス選定時(柴崎さん)に取り直すこと
- ストレージ費用(6枚×数百KB)は実質無視できる

*(B) 性格 — ChatGPT/Claudeと同じ「メモリー」方式の成長型*
- 岩田さんの要望は明確に「**ChatGPT/Claudeチャットの仕組みと同じように**」。
  静的(1回選んで固定)ではなく**成長型**。ただし新ベンダーは使わず **Dify+Supabase+Lovableのみ**で完結させる
- **設計(壁打ちで確定した内容)**:
  1. **1事実=1レコード**。長い要約文を書き換える方式ではなく、独立した短い事実を積み上げる
     (例: 「夜型で朝のトレーニングは苦手そう」)。1件ずつ削除・修正できることが肝
  2. **抽出は返信と同じDify呼び出し内**(別のcronジョブや2回目の呼び出しは作らない)。
     現状Difyの応答には既に `quick_replies` / `proposal` 等の付加情報が入っているので、
     そこに `new_memories: [{key, content}]` を1つ足すだけで済む
  3. **矛盾は「上書き」**(ChatGPT/Claudeの実挙動に合わせて確定。当初案の「ゆるく許容」は不採用)。
     Difyが同じ話題だと判定できるよう、**記憶1件ごとに「話題のキー」を持たせ、キーが一致したら上書き**
     (例: `running_preference` というキーで内容だけ書き換わる)
  4. **ユーザーに見せるのは必須寄り**。ChatGPT/Claudeも設定画面に記憶一覧があり個別に消せる。
     この透明性が「見られてる感」ではなく信頼につながっている
- **担当分け**: テーブル新設・Dify出力の拡張・`ai-chat` の改修 = 柴崎さん / 記憶一覧UI = こちら
- **テーブル案**: `trainer_memories`(`id` / `user_id` / `key`(話題キー) / `content`(1事実) /
  `created_at` / `updated_at`、`unique(user_id, key)`、RLSは `auth.uid() = user_id`)
- **`ai-chat` 側**: `new_memories` を受けたら `(user_id, key)` でupsert。
  あわせて現在の記憶一覧をコンテキストへ含めてDifyに渡す(次回の矛盾判定に必要)。
  `trainer_style` を渡している既存の配管がそのまま使える
- **フロント側(こちらの担当)**: `/trainer` に「トレーナーが知っていること」一覧+1件ずつ削除。
  **一度実装したがrevert済み**(経緯は冒頭の注意書き参照)。指示が出たら `git show f1a6bd5` で復元できる。
  実装時の注意: `trainer_memories` は生成型(`integrations/supabase/types.ts`)に無いため
  `any` キャストが必要。テーブル作成後に型を再生成すれば外せる
- ⚠️ **留保**: `trainer_style` がDifyに渡っているのは確認済みだが、
  **Dify側のプロンプトが実際にどこまで口調に反映しているかは未確認**。
  もし渡すだけで効いていないなら、パーソナライズ強化は「お金」ではなく
  「Difyのプロンプト設計の工数」として発生する
