...
最終更新: 2026-07-30(コミット bee69f2 時点。両リポジトリ完全同期済み)
...
- ChatGPT iOSアプリ風レイアウト。開くと最初にチャット
- **ヘッダー**: 左=メニュー(サイドバー)、右=⚙のみ。⚙タップで**歯車が回転しながら設定ページが円形に展開**
  ...
  ※旧「・・・メニュー」(会話のピン留め/共有/削除)と「新規会話」ボタンは削除済み。
  該当機能は現在UIから到達不可(menuOpen/menuPopover等のデッドコードがChat.tsxに残存。復活させるなら
  サイドバー履歴の長押しメニュー等で)。新規会話はサイドバーの「チャット」ボタンから
...
- **入力バー**: [ギャラリー添付] [カメラ] [textarea] [マイク(未実装トースト)] [送信]。+ボタンは廃止
...
- **サーバー履歴同期**(`lib/serverConversations.ts`): ログイン後 `ai_messages` から履歴を取得し、
  端末に無い分をサイドバーへ合成(別デバイスで履歴が見えない問題の対策)。1時間空きでセッション分割、
  ID+内容×時刻近似で重複排除、同時刻のuser/assistantはuser先。履歴表示はテキストのみ(カードのpayloadは
  サーバーに無い)。会話の区切りはサーバーに保存されないため元端末と完全一致はしない
- **既知の重要修正**: sendMessageは会話更新に必ず `setConvs(prev=>...)` を使う(スナップショット上書きで
...
- 会話のピン留め/共有/削除UIが現在到達不可(・・・メニュー削除のため)。要望があれば復活方法を提案
- 音声入力(マイクボタン)は未実装(トーストのみ)
